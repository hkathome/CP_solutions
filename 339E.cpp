...
ll query(int node, int s, int e, int l, int r) {
    ...
    int mid = s + (e - s) / 2;
    ll ans = max(query(2 * node + 1, s, mid, l, r), query(2 * node + 2, mid + 1, e, l, r));
...
void solve(){
    ...
    update(0,0,N-1,0,0);
    for(int i=1;i<=n;i++){
        int l=query(0,0,N-1,max(0,v[i]-d),v[i]);
        int r=query(0,0,N-1,v[i],min(N-1,v[i]+d));
        dp1[i]=1;
        if(l>=0 && l<i){
            dp1[i]=dp1[l]+1;
        }
        if(r>=0 && r<i){
            dp1[i]=max(dp1[i],dp1[r]+1);
        }
        dp2[i]=max(dp2[i-1],dp1[i]);
        update(0,0,N-1,v[i],i);
    }
    cout<<dp2[n]<<"\n";
}
int main() {
