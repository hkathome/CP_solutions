...
void nodes_toPath(vector<pii>nodes){
    reverse(all(nodes));
    for(int i=1;i<nodes.size();i++){
        while(nodes[i-1].ff<nodes[i].ff){
            cout<<"D";
            nodes[i-1].ff++;
        }
        while(nodes[i-1].ss<nodes[i].ss){
            cout<<"R";
            nodes[i-1].ss++;
        }
    }
...
seg.resize(4*W+4,0);
vector<pii>coins(n);
vector<int>adj[H+1],dir[H+1],dp[H+1];
for(int i=0;i<n;i++){
    cin>>coins[i].ff>>coins[i].ss;
    adj[coins[i].ff].pb(coins[i].ss);
}
for(int i=1;i<=H;i++){
    sort(all(adj[i]));
    int m=adj[i].size();
    ...
    for(int j=0;j<adj[i].size();j++){
        ...
        int ans1=query(0,0,W,0,adj[i][j]);
        if(ans1>dp[i][j]){
            dir[i].pb(1);
            dp[i][j]=ans1+1;
        }
    ...
    for(int j=0;j<adj[i].size();j++){
        update(0,0,W,adj[i][j],dp[i][j]);
    }
}
int ans=query(0,0,W,0,W);
cout<<ans<<"\n";
int cur=-1;
vector<pii>nodes;
nodes.pb({H,W});
...
for(int i=H;i>=1;i--){
   // dbgm(i,r);
    int idx=upper_bound(all(adj[i]),r)-adj[i].begin();
    idx--;
    if(idx>=0 && dp[i][idx]==ans){
       // dbgm(idx,adj[i][idx]);
        nodes.pb({i,adj[i][idx]});
        ans--;
        r=adj[i][idx];
        while(idx>0 && dir[i][idx]==0){
            idx--;
            nodes.pb({i,adj[i][idx]});
            r=adj[i][idx];
            ans--;
