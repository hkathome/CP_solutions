...
int dfs1(int p,int s,int h){
    int mxi=h;
    trav(z,adj[s]){
        if(z!=p){
            mxi=max(mxi,dfs1(s,z,h+1));
        }
    }
    dp[mxi+1]++;//if h>mxi we will have to remove this node;
    return mxi;
}
void solve(){
cin>>n;
adj.clear();adj.resize(n+1);
ht.clear();ht.resize(n+2,0);
dp.clear();dp.resize(n+2,0);
for(int i=1;i<n;i++){
    int u,v;
    cin>>u>>v;
    adj[u].pb(v);
