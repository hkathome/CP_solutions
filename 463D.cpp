...
v.resize(k+1);
adj.resize(k+1);
vector<int>lst(n+1,-1),fst(n+1,mod);
for(int i=1;i<=k;i++){
    v[i].resize(n+1,-1);
    adj[i].resize(n+1);
    for(int j=1;j<=n;j++){
        cin>>v[i][j];
        adj[i][v[i][j]]=j;
        lst[v[i][j]]=max(lst[v[i][j]],j);
        fst[v[i][j]]=min(fst[v[i][j]],j);
    }
}
//dbg(lst);dbg(fst);
...
for(int i=1;i<=n;i++){
    ...
    for(int i1=1;i1<=k;i1++){
        set<int>st;
        if(lst[v[i1][i]]==i && !st.count(v[i1][i])){
            st.insert(v[i1][i]);
            for(int k1=0;k1<=n;k1++){
                bool f=true;
                for(int l1=1;l1<=k;l1++){
                    if(adj[l1][k1]>=adj[l1][v[i1][i]]){
                       f=false;
                       break;
                    }
                }
                if(f){
                    dp[1][v[i1][i]]=max(dp[1][v[i1][i]],dp[0][k1]+1);
                }
            }
        }
    }
