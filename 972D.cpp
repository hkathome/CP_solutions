 ...
 //   debug(pre[0]);
  int n1=gs.size();
   for(int i=0;i<gs.size();i++){
        ...
        int ans1=0;
        if((l+1)<=(r-1)){
          int g1=query(seg2,0,0,n-1,l+1,r-1);
          if(g1%g!=0){
            continue;
          }
         int g2=gcd(query(seg1,0,0,n-1,l+1,r-1),gcd(pre[1][l],suf[1][r]));
          ans1=g+g2;
        }
        else{
            ans1=g+pre[1][n-1];
    ...
    for(int i=0;i<gs.size();i++){
       int  g=gs[i];
        ...
        int ans1=0;
        if(l+1<=r-1){
        int g1=query(seg1,0,0,n-1,l+1,r-1);
            if(g1%g!=0){
                continue;
            }
            //int g2=gcd(g,g1);
            g1=gcd(query(seg2,0,0,n-1,l+1,r-1),gcd(pre[0][l],suf[0][r]));
            ans1=g1+g;
        }
        else{
            ans1=g+pre[0][n-1];
   ...
   ll ct=0;
// if(pre[1][n-1]==pre[0][n-1] && ans==2*pre[1][n-1]){
//     ct=1ll*n*(n+1)/(1ll*2);
//     cout<<ans<<" "<<ct<<"\n";
//     return;
// }
