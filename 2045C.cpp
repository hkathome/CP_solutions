...
void solve(){
    ...
    for(int i=n2-2;i>=0;i--){
        arr[tt[i]-'a']=max(arr[tt[i]-'a'],i);
    }
    vector<pii>vv;
    for(int i=1;i<n1;i++){
        if(arr[ss[i]-'a']!=-1){
            vv.pb({i,arr[ss[i]-'a']+1});
        }
    }
    //dbg(vv);
    sort(all(vv),[&](pii a,pii b){
        return (n2-1-a.ss)+a.ff<(n2-1-b.ss)+b.ff;
    });
    //dbg(vv);
    if(vv.empty()){
        cout<<"-1\n";
    ...
    string anss="";
    for(int i=0;i<=vv[0].ff;i++){
        anss+=ss[i];
    }
    for(int i=vv[0].ss;i<n2;i++){
        anss+=tt[i];
    }
    cout<<anss<<n_l;
}
