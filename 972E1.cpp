...
int f(int idx,int r,int c){
    ...
    ans|=(f(idx,r+1,c)|f(idx,r,c+1));//if I don't choose this submatrix
    return dp[idx][r][c]=ans; //I will know if = works first or return
}
void solve(){
    cin>>l>>n>>m;
    a.clear();a.resize(l+1);
    int mxi=0;
    for(int i=1;i<=l;i++){
        cin>>a[i];
        mxi=max(mxi,a[i]);
    }
    mat.clear();mat.resize(n+1,vector<int>(m+1));
    for(int i=1;i<=n;i++){
        for(int j=1;j<=m;j++){
            cin>>mat[i][j];
        }
    ...
    for(int i=1;i<=7;i++){
    dp[i].clear();
    dp[i].resize(n+1,vector<int>(m+1,-1));
    }
    int x=0;
    for(int i=1;i<=l;i++){
        if((x>>(a[i]-1))&1){
