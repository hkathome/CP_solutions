...
void solve(){
    ...
    houses.resize(n);
    for(int i=0;i<n;i++){
        cin>>houses[i].ff>>houses[i].ss;
    }
    map<ll,set<ll>>Mp1,Mp2;
    for(int i=0;i<n;i++){
        Mp1[houses[i].ff].insert(houses[i].ss);
        Mp2[houses[i].ss].insert(houses[i].ff);
    }
    set<pii>st;
    for(int i=0;i<m;i++){
        char w; ll k;
        ...
        if(w=='L'){
            x-=k;
            auto it=Mp2[y].lower_bound(x);
            while(it!=Mp2[y].end() && *it<=(x+k)){
                st.insert({*it,y});
                auto it1=it;
                it++;
                Mp2[y].erase(it1);
            ...
            x+=k;
                ...
                auto it=Mp1[x].lower_bound(y);
            while(it!=Mp1[x].end() && *it<=(y+k)){
                st.insert({x,*it});
                auto it1=it;
                it++;
                Mp1[x].erase(it1);
