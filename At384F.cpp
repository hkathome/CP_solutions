...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        while(x%2==0){
            assert(j<M);
            ans1+=(1ll*arr[j][0]+1ll*arr[j][1]*x);
            j++;
            x/=2;
        }
        //j is max_power of 2 in x
        arr[j][0]+=x;
        arr[j][1]++;
        adj[j].pb(x);
        j++;
        ...
        while(j<M){
            p*=1ll*2;
            ans2+=(1ll*p*arr[j][0]+1ll*arr[j][1]*x);
            j++;
        }
     //   dbgm(ans1,ans2);
        ans+=(ans1+ans2+f(x));
    ...
    for(int i=0;i<M;i++){
      //  dbg(adj[i]);
        unordered_map<int,ll,custom_hash>hs[M];
        unordered_map<int,ll,custom_hash>hs1[M];
        trav(z,adj[i]){
            ll p=(1<<(M-1));
            ll j=M-1;
            ll ans1=0;
            ...
            while(j>=0){
                ll ans2=0;
                if(hs[j].count((p-z%p)%p)){
                    int cc=(p-z%p)%p;
                    ans2=hs[j][(p-z%p)%p]+1ll*hs1[j][cc]*z;
                }
                hs[j][z%p]+=z;
                hs1[j][z%p]++;
                ans2-=suf;
