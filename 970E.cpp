...
int even(string &s){
    ...
    for(int i=0;i<26;i++){
        mxi=max(mxi,arr1[i]);
        mxi2=max(mxi2,arr2[i]);
    }
    return s.size()-mxi-mxi2;
}
void solve(){
    int n;
    cin>>n;
    ...
    cin>>s;
    if(n&1){
        vector<vector<int>>pre1[2];
        pre1[0].resize(n+1,vector<int>(26,0));
        pre1[1].resize(n+1,vector<int>(26,0));
        vector<vector<int>>suf[2];
        suf[0].resize(n+1,vector<int>(26,0));
        suf[1].resize(n+1,vector<int>(26,0));
        for(int i=0;i<n;i++){
            for(int j=0;j<26;j++){
                pre1[0][i][j]=(i>0?pre1[0][i-1][j]:0);
                pre1[1][i][j]=(i>0?pre1[1][i-1][j]:0);
            }
            pre1[i%2][i][s[i]-'a']++;
        }
        for(int i=n-1;i>=0;i--){
