...
string s;
vector<ll>v;
map<char,int>Mp={{'h',3},{'a',2},{'r',1},{'d',0}};
const string ff="hard";
void solve(){
    cin>>n>>s;
    v.resize(n);
    for(int i=0;i<n;i++){
    ...
    for(int i=n-1;i>=0;i--){
        dp[i]=dp[i+1];
        if(ff.find(s[i],0)!=string::npos){
            dp[i][Mp[s[i]]]=dp[i+1][Mp[s[i]]]+v[i];
            if(s[i]!='h'){
            dp[i][Mp[s[i]]+1]=min(dp[i+1][Mp[s[i]]+1],dp[i+1][Mp[s[i]]]);
            }
        }
    }
    cout<<min({dp[0][0],dp[0][1],dp[0][2],dp[0][3]})<<n_l;
}
int main() {
    #ifndef ONLINE_JUDGE
