...
bool ok(int t){
    ...
    for(int i=0;i<n;i++){
        //zi*ti+yi,zi*ti+yi
        int m11=((t)/(zz[i]*tt[i]+yy[i]));
        int t2=t-m11*(zz[i]*tt[i]+yy[i]);
        int m2=min(zz[i],t2/tt[i]);
        m11*=zz[i];
        m11+=m2;
        m1+=m11;
        vv.pb(m11);
...
for(int i=0;i<n;i++){
    int t,y,z;
    cin>>t>>z>>y;
    tt.pb(t);yy.pb(y);zz.pb(z);
}
int l=-1;//won't be able to inflate m baloons in l time
int r=15000*200;
while(r-l>1){
    int mid=l+(r-l)/2;
    if(ok(mid)){ //if ok(mid)==1 then ok(mid+1)==1
        r=mid;
    }
    else{
        l=mid;
