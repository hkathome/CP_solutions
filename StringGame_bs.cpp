...
int n;
vector<int>a;
bool isSubsequence(string pp,string ss){
//returns true if pp is a subsequence of ss
int l1=0;
int l2=0;
while(l1<ss.size() && l2<pp.size()){
    if(ss[l1]==pp[l2]){
        l1++;
        l2++;
    }
...
void solve(){
 ...
 //   debug(ok(2));
    int l=-1;//l is good
    int r=n-1;// r is bad, we can't delete all
    while(r-l>1){
        int md=(l+r)/2;
        if(ok(md)){
            l=md;
