...
chota.clear();bada.clear();
chota.resize(n+1);bada.resize(n+1);
//{i,(i+1)%n}-i
//{1,2}-1, {2,3}-2,....,{n-1,n}->n-1,(n,1)->n
for(int i=2;i<=n;i++){
    int x;
    cin>>x;
    chota[i]=i-1;   //{i-1,i};
...
//how ans changes-> when xth edge is chosen, we need to check and update 2 paths, 
//if no of edges rem
int x;
ll w;
dfs(-1,1);
int ct=0;//counts the no. of known paths; all other paths can have max wt.
ll sum=0;//the sum of known paths
vector<ll>paths(n+1,0);
vector<int>pathEdges(n+1,0);
for(int i=2;i<=n;i++){
    int u,v;
    u=chota[i];
    v=i;
